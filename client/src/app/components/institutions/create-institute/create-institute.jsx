...
import { cityscapes } from '../image'
import { Button } from '@stories'
import { selectGroupInstitutions } from '@entities/select-style'

export default function CreateInstitute({ setModal, burger }) {
  ...
  return (
    <div className='flex items-start justify-between mt-20 w-full pr-28 pl-10 2xl:pl-24 2xl:pr-24'>
      <Result resultDot={resultDot} />
      <div className='flex flex-col items-center' style={{ width: '25.25rem', minHeight: '35.375rem' }}>
        <h2 className='font-bold text-2xl'>Create an Institution</h2>
        <p className='font-medium text-sm mt-4 whitespace-pre'>
          We start by creating an instituion and then do the interesting stuff.
        </p>
        <img src={cityscapes} alt='cityscapes' className='mt-11' />
        <div className='mt-14'>
          <p className='text-dot-gray font-bold text-xs'>INSTITUTION NAME</p>
          <input
        ...
        <div className='mt-5'>
          <p className='text-dot-gray font-bold text-xs mb-3'>COUNTRY</p>
          <Select options={options} value={value} onChange={changeHandler} styles={selectGroupInstitutions} />
        </div>
        <Button
          text='Save and continue'
          className='flex items-center justify-center mt-6 mb-20 medium-after'
          customStyle={{ width: '25.25rem', height: '2.5rem' }}
          onClick={() => history.push('/institutions/step-one')}
        />
      </div>
...
const resultDot = [
  { dot: 'active', text: 'Getting Started' },
  { dot: 'unactive', text: 'Address' },
